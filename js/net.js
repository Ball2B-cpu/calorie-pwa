...
ข้อความที่ผู้ใช้พิมพ์ทุกบรรทัด = ของที่กินจริงคนละที่/คนละเสิร์ฟ แม้ชื่อซ้ำหรือคล้ายบรรทัดอื่น (มื้อเดียวอาจกินของเดิมหลายรอบคนละเวลา) → ตอบ 1 รายการต่อ 1 บรรทัด ห้ามรวม/ตัดซ้ำ/ทิ้งบรรทัดใด · ถ้าสงสัยว่าพิมพ์ซ้ำโดยไม่ตั้งใจ ยังนับครบทุกบรรทัด (ห้ามลดยอดรวม) แล้วเขียนแจ้งใน warnings ให้คนตรวจตัดสิน
...
// 21 ก.ย. รอบสอง: "ต้มยำอีกหนึ่งส่วนสาม" ไม่เข้า regex เดิม → เดาใหม่ 40 kcal ทั้งที่มื้อเช้า ⅓ เดียวกัน = 152
//   → จับ "อีก" + ลักษณนาม/จำนวน ทุกแบบ (false positive แค่เปลือง token นิดเดียว ยอมได้)
const BACKREF_RE = /ที่เหลือ|ครึ่งหลัง|ครึ่งที่|ส่วนสาม|ส่วนสี่|ส่วนที่เหลือ|อีก\s*(ครึ่ง|หนึ่ง|ส่วน|ถุง|ชิ้น|กล่อง|จาน|ชาม|ห่อ|ถ้วย|แก้ว|ที่|รอบ|[\d๑-๙])|เหมือนเดิม|เหมือนเมื่อ|เหมือนมื้อ|เมื่อเช้า|เมื่อกลางวัน|มื้อเช้า|มื้อกลางวัน/;
